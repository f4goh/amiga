/**
 * example_05.c - sprite priorities demonstration
 * This example program displays a 15 color and a 3 color sprite in a fishtank
 * to demonstrate priorities between sprites and the background
 */
...
// the index in the copper list for setting priorities in BPLCON2
#define COPLIST_IDX_BPLCON2_VALUE (COPLIST_IDX_DIWSTOP_VALUE + 4)
...
// for switching sprite-playfield priorities
static bplcon2_values[4] = { 0x40, 0x48, 0x50, 0x58 };
static int prio_idx = 0;
                                           ...
                                           __reg("a1") APTR handler_data)
{
    ...
    if (result->ie_Class == IECLASS_RAWMOUSE) {
        if (result->ie_Code == IECODE_LBUTTON) {
            ...
            prio_idx = (prio_idx + 1) % 4;
            coplist[COPLIST_IDX_BPLCON2_VALUE] = bplcon2_values[prio_idx];
        }
        return NULL;
    }
    Permit();
...
static struct Ratr0SpriteSheet goby_l2r, goby_r2l, nemo_l2r, nemo_r2l;

// Composite sprites for the 2 directions
static ULONG goby_sprite1[4], goby_sprite2[4], nemo_sprite1[2], nemo_sprite2[2];
...
// set position for a composite sprite (attached, 32 pixel wide)
...
static void cleanup(void)
{
    ...
    ratr0_free_spritesheet_data(&nemo_l2r);
    ratr0_free_spritesheet_data(&nemo_r2l);
    reset_display();
}
...
int main(int argc, char **argv)
{
    ...
    // Read 4 sprite sheets for 2 characters, 2 directions (left-to-right and right-to-left)
    ...
    if (!ratr0_read_spritesheet("nemo32x16x2_l2r.spr", &nemo_l2r)) {
        puts("could not read nemo sprite 1");
        cleanup();
        return 1;
    }
    if (!ratr0_read_spritesheet("nemo32x16x2_r2l.spr", &nemo_r2l)) {
    ...
    init_composite_sprite(nemo_sprite1, 2, &nemo_l2r);
    init_composite_sprite(nemo_sprite2, 2, &nemo_r2l);
    ...
    ULONG *curr_goby = goby_sprite1, *curr_nemo = nemo_sprite1;
    set_sprite_ptrs(curr_nemo, 2, COPLIST_IDX_SPR0_PTH_VALUE);  // nemo uses 2 sprites
    set_sprite_ptrs(curr_goby, 4, COPLIST_IDX_SPR0_PTH_VALUE + 8);

    // set sprite colors, goby palette covers nemo palette
    for (int i = 1; i < 16; i++) {
        coplist[COPLIST_IDX_COLOR00_VALUE + ((16 + i) * 2)] = goby_l2r.palette[i];
    }

    // and set the sprite position
    UWORD goby_x = 225, goby_y = 142, goby_height = 21;
    UWORD nemo_x = 235, nemo_y = 133, nemo_height = 16;
    if (is_pal) {
        goby_y += 56;
        nemo_y += 56;
    }
    set_sprite_pos_att32(curr_goby, goby_x, goby_y, goby_y + goby_height);
    set_sprite_pos_32(curr_nemo, nemo_x, nemo_y, nemo_y + nemo_height);
    // SET SPRITE DATA END

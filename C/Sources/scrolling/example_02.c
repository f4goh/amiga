/**
 * example_02.c - horizontal scrolling example
 * Shows horizontal scrolling
 * of an image larger than the display window
 * by a combination of setting values in BPLCON1 and
 * updating the bitmap pointers
 */
...
static UWORD __chip coplist[] = {
    ...
    // DDFSTRT was decreased by 8 bus cycles, so we have to adapt
    // the bitplane modulos, too
...
int main(int argc, char **argv)
{
    ...
    int x_offset = 0;
    int x_inc = SPEED;
    int num_pixels_shift = 0;
    int num_words_skip = 0;

    // keep in mind: the amount given in BPLCON1 is actually the number of pixels
    // the image will be shifted to the right
    UWORD delay_mask = 0;
    ...
    while (!should_exit) {
        ...
        addr = (ULONG) background.imgdata + num_words_skip * 2;
        for (int i = 0; i < background.header.bmdepth; i++) {
            coplist[coplist_idx] = (addr >> 16) & 0xffff;
            coplist[coplist_idx + 2] = addr & 0xffff;
            coplist_idx += 4;
        ...
        // increment x-position and reverse direction if maximum reached
        x_offset += x_inc;
        if (x_offset < 0 || x_offset > MAX_X_OFFSET) {
            x_inc = -x_inc;
            x_offset += x_inc;
        }
    }
